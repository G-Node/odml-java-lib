...
public class Property extends Object implements Serializable, Cloneable, TreeNode {
	...
	public static Object[] columns = { "name", "id", "value", "uncertainty", "unit", "type", "defaultFileName",
			"valueDefinition", "propertyDefinition", "dependency", "dependencyValue" };
	public static int MATCH_ERROR = -1, MATCH_NO = 0, MATCH_FIRST_CONFLICT_LAST_MATCH = 5, MATCH_INITIALS_ONLY = 10,
			MATCH_FIRST_OR_LAST_ONLY = 20, MATCH_FIRST_INITIAL_LAST = 30, MATCH_FIRST_LAST = 50, MATCH_EXACT = 50;
	...
	public Property(Vector<Object> data) throws Exception {
		this((String) data.get(0), (String) data.get(1), data.get(2), (String) data.get(3), data.get(4), (String) data
			.get(5), (String) data.get(6), (String) data.get(8), (String) data.get(7), (String) data.get(9),
	 ...
	 * @throws Exception
	 */
	public Property(String name, Vector<Value> values, String definition, String dependency, String dependencyValue,
	 ...
	 * @throws Exception
	 */
	public Property(String name, String id, Object value, String unit, Object uncertainty, String type,
			String defaultFileName, String definition, String valueDefinition, String dependency,
	 ...
	 *            {@link String}
	 * @param values
	 *            {@link Vector} of Objects
	 * @param ids
	 *            {@link Vector} of Strings
	 * @param unit
	 *            String
	 * @param uncertainties
	 *            {@link Vector} of Objects
	 ...
	 * @throws Exception
	 */
	public Property(String name, Vector<Object> values, Vector<String> ids, String unit, Vector<Object> uncertainties,
			String type, Vector<String> fileNames, String definition, Vector<String> valueComments, String dependency,
			String dependencyValue, URL mapping) throws Exception {
		if (name == null) {
			...
			throw new PropertyNameLikePathException(
				"Could not create property! 'name' must not be like a path (e.g. contain '/')!");
		} else if (values.size() < uncertainties.size() || values.size() < valueComments.size()
				|| values.size() < fileNames.size()) {
			throw new Exception(
				"Could not create property! There must not be more errors or definitions than there are values.");
		}
		...
		for (int i = 0; i < values.size(); i++) {
			if (uncertainties.size() > 0)
				tmpUncertainties = uncertainties.get(i);
			if (fileNames.size() > 0)
				tmpDefaultFileNames = fileNames.get(i);
			if (valueComments.size() > 0)
				tmpValueComments = valueComments.get(i);
			if (ids.size() > 0)
				tmpIds = ids.get(i);
			try {
				logger.debug("having a look at the single value components: \n\tvalue itself: \t\t"
						...
						+ tmpUncertainties + "\n\tthe type: \t\t" + type + "\n\tdefaultFileName: \t"
						+ tmpDefaultFileNames + "\n\the valueComment: \t" + tmpValueComments + "\n\tfinally the id: \t"
						+ tmpIds);
				Value oneValue = new Value(values.get(i), unit, tmpUncertainties, type, tmpDefaultFileNames,
					tmpValueComments, tmpIds);
				theValues.add(oneValue);
		...
		for (int i = 0; i < otherProperty.valueCount(); i++) {
			if (this.values.contains(otherProperty.getValue(i))) {
				...
				if (mergeOption == Section.MERGE_COMBINE) {
					this.addValue(otherProperty.getValue(i), otherProperty.getValueReference(i), otherProperty
						.getValueUncertainty(i), otherProperty.getFilename(i), otherProperty.getValueDefinition(i));
	...
	public void compareToTerminology(Property termProp) {
		if (definition != null && !definition.isEmpty()) {
			if (!this.definition.equalsIgnoreCase(termProp.getDefinition())) {
				logger.warn("Property: " + this.getName()
	...
	private void mergeValue(int thisValueIndex, Property otherProperty, int otherValueIndex, int mergeOption) {
		...
		case Section.MERGE_THIS_OVERRIDES_OTHER:
			...
			if (this.getValueDefinition(thisValueIndex) == null) {
				this.setValueDefinitionAt(otherProperty.getValueDefinition(otherValueIndex), thisValueIndex);
			}
			// uncertainty
			if (this.getValueUncertainty(thisValueIndex) == null) {
				this.setValueUncertaintyAt(otherProperty.getValueUncertainty(otherValueIndex), thisValueIndex);
			}
			// defaultFileName
			if (this.getFilename(thisValueIndex) == null) {
				this.setDefaultFileNameAt(otherProperty.getFilename(otherValueIndex), thisValueIndex);
			}
			// id
			if (this.getValueReference(thisValueIndex) == null) {
				this.setValueIdAt(otherProperty.getValueReference(otherValueIndex), thisValueIndex);
			}
			break;
		case Section.MERGE_OTHER_OVERRIDES_THIS:
			...
			if (this.getFilename(thisValueIndex) == null) {
				this.setDefaultFileNameAt(otherProperty.getFilename(otherValueIndex), thisValueIndex);
			} else if (otherProperty.getFilename(otherValueIndex) != null) {
				this.setDefaultFileNameAt(otherProperty.getFilename(otherValueIndex), thisValueIndex);
			}
	...
	public String getUnit() {
		if (values.size() > 1) {
			logger.error("! property '" + this.getName()
					+ "' has more than one value > index must be specified to know which unit shall be" + " returned!");
			return null;
		}
		return getUnit(0);
	}
	 ...
	 * 
	 * @param index
	 *            {@link Integer}: the index to specify which defaultFileName shall be returned (i.e. from which value)
	 * @return {@link String}: returns the mimetype of a certain propery. Returns an empty String if no mimeType stored
	 *         or null if the index is out of range.
	 */
	 ...
	 * @param index
	 *            {@link Integer}: the index of the value to which the DefaultFileName belongs.
		...
		if (!firstName1.isEmpty() && !firstName2.isEmpty()) {
			...
			if (firstName1.isEmpty() && firstName2.isEmpty()) {
				...
				if (firstName1.isEmpty()
						&& (lastName1.equalsIgnoreCase(lastName2) || lastName1.equalsIgnoreCase(firstName2))) {
					return MATCH_FIRST_OR_LAST_ONLY;
				} else if (firstName2.isEmpty()
						&& (lastName2.equalsIgnoreCase(lastName1) || lastName2.equalsIgnoreCase(firstName1))) {
					return MATCH_FIRST_OR_LAST_ONLY;
				} else {
					return MATCH_NO;
				}
