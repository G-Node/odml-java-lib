...
public class Section extends Object implements Serializable, TreeNode {
	...
	public static String checkTypeStyle(String type) {
		...
		while (type.contains(" ")) {
			...
			logger.info("Invalid section type: types must not contain blanks, all blanks were replaced!");
		}
		...
		for (int i = 0; i < subsections.size(); i++) {
			...
			if (subsectionType.equalsIgnoreCase(type)) {
				temp.add(subsections.get(i));
					...
					&& subsectionType.substring(0,subsectionType.indexOf("/")-1).equalsIgnoreCase(type)) {
				temp.add(subsections.get(i));
			}
		}
		if (temp.size() == 0) {
	...
	public void validate() {
		...
		logger.info("Validating section " + this.getPath() + " against terminology located in repository " 
