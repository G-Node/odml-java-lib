...
/* TODO (CES 30.07.2010): checking all get-/set-/add-/delete- Section or Property cases with different paths!
 * (e.g. having '/sec' or '/prop' not working properly until now!)
 * normal cases work well, purgeEmptySections working, checkDatatType finally working correct
 * still needs more testing: mapping, linkdSection stuff and terminology use (only simple cases tested so far, 
 * no nested stuff yet)
 */

/**
 * The {@link Section} class defines the odML section, one of the two core elements that can contain odMLProperties
 * - if it is not the root of the odMLTree - as well as section to built a tree structure.
 * A Section is defined by its name and may carry a description. Furthermore a section may 
 * contain one terminology which contains property definitions and which can be used to 
 * validate the properties. Properties defined in the terminology will be checked for consistency
 * and missing fields like type or unit will be added from the terminology.
 * 
...
public class Section extends Object implements Serializable, TreeNode{
	static Logger logger = Logger.getLogger(Section.class.getName()); // for logging
	private static final long serialVersionUID = 145L; 
	public static final int MERGE_THIS_OVERRIDES_OTHER=0,MERGE_OTHER_OVERRIDES_THIS=1, MERGE_COMBINE=2;
	private String type = null, definition = null, name = null, reference = null;
	...
	//*****************************************************************
	//	/**
	//	 * constructing a Section out of a terminologyURL, i.e. getting name and defs from there
	//	 */
	//	public Section(URL termURL) throws Exception{
	//		this("dummy"); 
	//		Reader readTerm = new Reader(termURL);
	//		Section dummyTerm = readTerm.getRootSection();
	//		
	//		// usually terminology xml File starts with the odml-rootSection, that contains no info
	//		if(dummyTerm.getType() == null || dummyTerm.getType().isEmpty()){
	//			dummyTerm = readTerm.getRootSection().getSection(0);
	//		}
	//		// assigning all info from terminology to newly created Section
	//		this.type = dummyTerm.type;
	//		this.name = dummyTerm.getName();
	//		this.mappingURL = dummyTerm.mappingURL;
	//		this.definition = dummyTerm.definition;
	//		this.synonyms = dummyTerm.synonyms;
	////		this.sectionDependencies = dummyTerm.sectionDependencies;
	////		this.parentURLs = dummyTerm.parentURLs;
	//		this.termURL = termURL;
	//		this.terminology = dummyTerm;
	//		rootSec = this;
	//	}
	/**
	 * The default constructor. Creating the root for the odmlTree, i.e. no name is necessary and no parent 
	 * must be given. 
	 */ 
	public Section(){
	 ...
	 * @param parent {@link Section}: the parent of this section.
	 ...
	 * @param name - {@link String}:
	 * @param type - {@link String}:
	 * @param definition - {@link String}: the definition of this section.
	 */
	public Section(Section parent, String name, String type, String reference,  String definition) throws Exception {
		this(parent, name, type, reference, definition, null);
	}
	/**
	 * 
	 * @param parent {@link Section}: the parent of this section.
	 * @param name {@link String}: the section name.
	 * @param type {@link String}: the section type.
	 * @param definition {@link String}: a description, can be null.
	 * @param baseURL {@link URL}: the URL of the underlying terminology.
	 * @throws Exception
	 */
	public Section(Section parent, String name, String type, String reference, String definition, URL baseURL) throws Exception{
		this(parent,name,type,reference,definition,baseURL, null);
	}
	/**
	 * Constructor that creates a new section with all the possible information (name, description, terminology
	 *  and mappingURL. Also parentURLs (logical parent) and partenSections are included)
	 * @param parent
	 * @param type {@link String}: the name of the Section, must not be null or empty
	 * @param definition
	 * @param baseURL
	 * @param mappingURL
	 * @throws Exception
	 */
	public Section(Section parent, String name, String type, String reference, String definition, URL baseURL, URL mappingURL) throws Exception{
		super();
		if(type == null){
		...
		else {	this.level = 0;	} // if no parent is given > this one is root, i.e. level = 0
	}
	/**
	 * Checks whether the given section type is in lower case, i.e. having one word beginning with
	 * an uppercase letter. If not generating CamelCase by deleting white-spaces and 
	 * putting letter of formerly new word to uppercase. If String is beginning with a 
	 * digit adding S_ for indicating a Section-type 
	 * @param type {@link String}: the type that shall be checked / converted to CamelCase
	...
	public static String checkTypeStyle(String type){	
		...
		while(type.contains(" ")){
			...
			//			type = type.substring(0,type.indexOf(" "))+type.substring(type.indexOf(" ")+1, type.indexOf(" ")+2).toUpperCase()+type.substring(type.indexOf(" ")+2);
	 ...
	 * dependencyURLs, mappingURL, terminologyURL, synonyms
	 * If not taking the info of the ordinary section and logging a warning
	 * @param termSec {@link Section}: the terminology section
	 * @param ordinarySec {@link Section}: the section of the user
	 */
	private void checkTerminologyConsistency(Section termSec, Section ordinarySec){
		if (termSec == null) return;
		if (ordinarySec == null) return;
		...
		if (termSec.getMapping() != null && (!termSec.getMapping().toString().isEmpty())){
			...
			else if (!termSec.getMapping().toString().equalsIgnoreCase(ordinarySec.getMapping().toString())){
		...
		if (termSec.getRepository() != null && (!termSec.getRepository().toString().isEmpty())){
			if(ordinarySec.getRepository() == null || (ordinarySec.getRepository().toString().isEmpty())){
				ordinarySec.setRepository(termSec.getRepository());
				logger.info("terminologyURL set to one used in terminology");
			} 
			else if (!termSec.getRepository().toString().equalsIgnoreCase(ordinarySec.getRepository().toString())){
				logger.warn("terminologyURL of checked section and of one in terminology different! " +
				"Using the one defined by user");
			}
		}
		// checking the synonyms // actually there are no synonyms in the terminology as this is the original term
		//		if (termSec.getSynonyms() != null && (!termSec.getSynonyms().isEmpty())){
		//			if(ordinarySec.getSynonyms() == null || (ordinarySec.getSynonyms().isEmpty())){
	...
	/**
	 * checking if the property is consistent to the given terminology of its parent-section 
	 * in aspects of dependency, dependency-value, nameDefinition, mappingURL, propSynonyms.
	 * If not taking the info of the ordinary property and logging a warning
	 * @param property {@link Property}: the property that shall be checked against the one defined in the terminology
	 * of its parent-section. if there is no terminology set or no such property in it, return is called
	 */
	private void checkTerminologyConsistency(Property property){
		if (this.terminology == null) return;
		if (this.terminology.propertyCount() <= 0) return;
		...
		if (termProp.getDependency() != null && (!termProp.getDependency().isEmpty())){
			if(property.getDependency() == null || (property.getDependency().isEmpty())){
				...
				logger.warn("dependency set to one used in terminology; user has to check correctness!");
			} 
		...
		if (termProp.getDependencyValue() != null && (!termProp.getDependencyValue().isEmpty())){
			if(property.getDependencyValue() == null || (property.getDependencyValue().isEmpty())){
				...
				logger.warn("dependencyValue set to one used in terminology; user has to check correctness!");
			} 
		...
		if (termProp.getMapping() != null && (!termProp.getMapping().toString().isEmpty())){
			...
			else if (!termProp.getMapping().toString().equalsIgnoreCase(property.getMapping().toString())){
				logger.warn("mappingURL of checked property and of one in terminology different! " +
				"Using the one defined by user");
			}
		}
		// checking the propSynonyms // actually there are no synonyms in the terminology as this is the original term
		if (termProp.getSynonyms() != null && (!termProp.getSynonyms().isEmpty())){
			if(property.getSynonyms() == null || (property.getSynonyms().isEmpty())){
		...
		for (int i = 0; i<property.valueCount(); i++){
			if (termProp.getValueIndex((property.getWholeValue(i)).getContent()) >= 0){
				Value fromTerm = termProp.getWholeValue(termProp.getValueIndex((property.getWholeValue(i)).getContent()));
				checkTerminologyConsistency(fromTerm, property.getWholeValue(i));
			}
		}
	}
	/**
	 * checking if the value is consistent to the given terminology-value of its parent-property 
	 * in aspects of unit, type, uncertainty, defaultFileName, valueComment, id 
	 * If not taking the info of the ordinary Value and logging a warning
	 * @param fromTerm {@link Value}: the value defined in the terminology
	 * @param value {@link Value}: the value of the user, that shall be checked
	 * return if one value == null
	 */
	private void checkTerminologyConsistency(Value fromTerm, Value value){
		...
		if (fromTerm.getUncertainty() != null && (!fromTerm.getUncertainty().toString().isEmpty())){
			if(value.getUncertainty() == null || (value.getUncertainty().toString().isEmpty())){
				...
				logger.warn("uncertainty set to one used in terminology; user has to check correctness!");
			} 
		...
		if (fromTerm.getFilename() != null && (!fromTerm.getFilename().isEmpty())){
			if(value.getFilename() == null || (value.getFilename().isEmpty())){
				...
				logger.warn("defaultFileName set to one used in terminology; user has to check correctness!");
			} 
		...
		if (fromTerm.getDefinition() != null && (!fromTerm.getDefinition().isEmpty())){
			if(value.getDefinition() == null || (value.getDefinition().isEmpty())){
				...
				logger.info("valueComment set to one used in terminology; user has to check correctness!");
			} 
		...
		// checking the id // actually independent from terminology as id dependent on e.g. local database
	 ...
	 * @param section - {@link Section}: the new section.
	...
	public int add(Section section){
		...
		if(section!= null){
			if(this.getSections(name).size()>0){
				logger.warn("There already exists a section with that name! Will append an index to the name!");
	...
	//	/**
	//	 * Adds a new subsection to this section. The new section will have the specified name. 
	//	 * @param name {@link String} The new section's name, can be a path.
	//	 * @param type {@link String} the section's type. If null or empty the type will be the same as the name.
	//	 * @return {@link Integer} the index of the new section
	//	 * (i.e. the number of subsections as added one is last), -1 when adding failed
	//	 */
	//	public int addSection(String name, String type){
	//		return this.addSection(name, type, type, null, null);
	//	}
	//	/**
	//	 * Adds a new subsection to this section. The new section will have the specified name and definition. 
	...
	//	 * @param definition - String: the definition of this subsection. Can be null or empty
	//	 * @param baseURL
	//	 * @param mappingURL
	...
	//	 */
	//	public int addSection(String name,String type, String definition, URL baseURL, URL mappingURL){
	//		try{
	//			//			if (name.contains("/")) { // name is a path as containing "/"
	//			//				String path = ensureValidPathEnding(name);
	//			//				if(type == null || type.isEmpty()){type = path.substring(path.lastIndexOf("/")+1);}
	//			//				sectionToAdd = new Section(this, type, path.substring(path.lastIndexOf("/")+1), 
	//			//						definition,baseURL, mappingURL);
	//			//				return addSection(path.substring(0,path.lastIndexOf("/")), sectionToAdd);
	//			//			}
	//			//			else { // name ordinary section name
	//			//			if(type == null || type.isEmpty()){type = name;}
	//			Section sectionToAdd = new Section(this, name, type, definition,baseURL, mappingURL);
	...
	/**
	 * Returns a section specified by it's path from the root (beginning with '/') or from
	 * the current section (beginning directly with the subsection-name)
	 * @param pathOfSections {@link String}: path to the wanted section
	...
	public Section getSection(String name){
		...
		if (temp != null && temp.size() > 1){
			logger.warn("Section.getSection: more than one subsection of this name exists > returning first occurence");
			return temp.get(0);
		}
	...
	/**
	 * Returns a {@link Vector} of subsections that match the name. Matching is 
	 * case-insensitive.<br/>
	 * 
	 * @param name - {@link String}: the name of subsections.
	...
	public Vector<Section> getSections(String name){
		...
		if(name != null){
			if(name.contains("/")){
				...
				logger.info("name for getSections actually a path > calling getSectionViaPath to search at right place");
				// actualParentsName is at penultimate position, ultimate position is name of wanted sections!
			...
			// ensure validness of name (i.e. if no loading character > adding S_ as also done so when adding Section
	 ...
	 * Method does not crawl through the rest of the tree. Use FindSectionByType, instead. 
	 * 
	...
	public Section getSectionByType(String type){
		...
		if(temp.size()>1){
			logger.warn("Section.getSectionByType: more than one subsection of this type exists > returning first occurence");
			found = getSectionsByType(type).get(0);
		}
		return found;
	}
	/**
	 * Returns a Vector of subsections of the requested type. Matching is 
	 * case-insensitive.
	...
	public Vector<Section> getSectionsByType(String type){
		...
		for (int i = 0;i<subsections.size();i++){
			...
			else if(subsections.get(i).getType().contains("/") && subsections.get(i).getType().startsWith(type)){
				temp.add(subsections.get(i));
			}
		}
		if(temp.size()==0){
	...
	/**
	 * findSection looks for a child section with the specified name. In contrast to getSection, findSection
	 * recursively crawls down all subsection. Returns the first occurrence! If name is a path this method is equivalent to
	 * getSection(name).
	 ...
	 * @param name {@link String} the name or path to look for.
	 * @return {@link Vector}<{@link Section}> All matching sections. In case of a path the vector contains 
	 * only one element. May be empty if no match found.
	 */
	 ...
	 * Section relations rate from children over siblings to parents, their siblings, grandparents, their siblings
	 * and so on...
	...
	public Section getRelatedSection(String type){
		Vector<Section> temp = findSectionsByType(type);
		if(temp.size()==0){
	...
	/**
	 * Returns all Sections that have the specified type and are have the strongest relation to 
	 * this section. Relation strengths decrease from children over siblings to parents, their siblings, grandparents, their siblings
	 * and so on... Sections with equal relation strength are returned. That means that they are siblings.
	...
	// TODO: copySection(fromPath, toPath) using the copy function to get a real second object, not only reference!
	/**
	 ...
	 * @param toRemove {@link Section} the section that should be deleted.
	...
	/**
	 * Removes the first subsection with this name (accordingly at this position in the tree with
	 * the name as the last section in path)
	...
	//	/**
	//	 * Method to remove a section specified by a path. Leading "/" means path beginning from root, 
	//	 * otherwise beginning from current section (e.g. subsec/subsubsec/secToRemove)
	//	 * @param pathOfSections
	//	 * @return {@link boolean}: true if removed successfully, false if no section with specified name existing
	//	 */
	...
	//		Section parentOfRemover = getActualSectionSpecifiedByPath(pathOfSections.substring(0, pathOfSections.lastIndexOf("/")), false);
	//		if(parentOfRemover == null) return false;
	//		return parentOfRemover.removeSection(pathOfSections.substring(pathOfSections.lastIndexOf("/")+1));
	//	}
	//*****************************************************************
	//**************			section basics				***********
	//*****************************************************************
	/**
	 * Updates the level of the according section and all it's subsections.
	 * Called when calling addSection(), as a Section can become subsection of
	 * another one when merging two files, etc.
	 */
	private void updateLevel(){
		if(this.getParent()== null){
			...
			this.level = ((Section)this.getParent()).getLevel()+1;
		}
	...
	/**
	 * Set the reference element of the {@link Section}. the reference can be used to 
	 * point to an entity e.g. defined in a database.
	 * 
	 * @param reference
	 */
	...
	public boolean setRepository(String url){
		try{
			...
			// appending the terminology-tree to the section and checking consistency
			//			Reader readTerm = new Reader(termURL);
			//			this.terminology = readTerm.getRootSection();
			//			// usually terminology xml File starts with the odml-rootSection, that contains no info
			//			if(this.terminology.getType() == null || this.terminology.getType().isEmpty()){
	 ...
	 * Uses resolveLink() recursively. Call it on 
	 */
	 ...
	 * @param property - {@link Property}: the property you want to add.
...
//		System.out.println("\tSection.add(Property): "+property.getName());
		if(this.level == 0 && this.type == null){
			logger.error("! property must not be added to the root section (level == 0 && name == null)!");
			return -1;	
		}
		if(property == null){
		...
		if(index>-1){
			if (properties.get(index).equals(property)){
				logger.error("! nothing added as identical property already existing"+
						"\n\tproperty details: "+property.toString());
				return index;
			}
			else{
	 ...
	 * If sections in path not existing yet, creating them. path beginning with '/' meaning 
	 * starting from the root.
	 * @param pathOfSections {@link String}: the path form the root, where the property 
	 * shall be added; different levels separated by "/", path ending on section-name!!
	 * @param property - {@link Property}: the property you want to add.
	 * @return {@link Integer}: the index of the added property in the properties vector or -1 if command failed.
	 */
	 ...
	 * @param name {@link String} the name of the new Property.
	 * @param value {@link Object} the new Property's value.
	...
	/**
	 * Method to remove a property specified by a path. Leading "/" means path beginning from root, 
	 * otherwise beginning from current section (e.g. /root/subsec/subsubsec/propToRemove)
	 * @param path
	...
	private boolean removePropertyViaPath(String path){
		...
		//parentOfRemover is at penultimate position in the path!
		Section parentOfRemover = getActualSectionSpecifiedByPath(path.substring(0, path.lastIndexOf("/")), false); 
		if(parentOfRemover == null) return false;
		return parentOfRemover.removeProperty(path.substring(path.lastIndexOf("/")+1));
	}
	/**
	 * Removes a property matching with its name from the properties that are
	 * stored in this section. Removes the first occurrence with this name and 
	...
	public boolean removeProperty(String name){
		...
		// if name contains "/" not only at first place, finding right parent and removing prop from there
		if (name.contains("/")){
			name = ensureValidPathEnding(name);
			if (name.substring(1).contains("/"))	return removePropertyViaPath(name);
			else {name = name.substring(1);} // TODO eigentlich von root entfernen! (auch wenn des meistens ned gehn sollte, weil dort keine props zulaessig sind!)
		}
		int place=-1;
	...
	/**
	 * Removes the property defined by its index the properties that are
	 * stored in this section. 
	 * 
	 * @param index {@link Integer} : the index of the property. With -1<index<getPropertyCount().
	...
	/**
	 * Returns a list of the property names in this section. Method does not traverse 
	...
	/**
	 * Returns the property given by a path (looking like /root/sec1/subsec1/property-name). 
	 * If sections of path not existing returning null.
	 * @param pathToProperty {@link String}: the path to the wanted property, ending with 
	 * the name of the property (if ending with '/' > deleting last character!)
	 * i.e. wanted path, not only the property-name itself (as there is another method getProperty (String name)
	...
	private Property getPropertyViaPath(String pathToProperty, boolean resolveLink){
		...
		// splitting path in sectionPath (until last '/') & propertyName ('rest')
		...
		logger.debug("total path: "+pathToProperty+", path to section: "+pathToParentSec+" and propName: "+propName);
		...
		if (newParent.getType() != null)
			logger.debug("found parentsec of property '"+propName+"' via path with name: '"
					+newParent.getType()+"' and total path: '"+pathToParentSec+"'");
		return newParent.getProperty(propName, resolveLink);
	}
	/**
	 * Returns the first property stored in this section that matches the name. Name matching is case-insensitive.
	 * The name can also be a path looking like /section/section/property. Leading "/" means beginning from root, 
	 * leading section-name meaning path beginning from current section.
	 * In case no direct match is found method also checks the synonyms. The first match is returned.
	 * Identical for search via path, just changing position in tree.
	...
	public Property getProperty(String name){
		...
		if(name != null && !name.isEmpty()){
			...
			else 	p = getProperty(name, true); // ordinary property name
		}
		return p;
	}
	/**
	...
	/**
	 * Internal method to retrieve a property. If the property is not found in this section itself the 
	 * a possible link will be resolved and the property search starts again. 
	 * 
	 * @param name {@link String} the name of the searched property.
	...
	private Property getProperty(String name,boolean resolveLink){
		...
		if (name.contains("/")){
			...
			if(p == null && resolveLink && this.resolveLink() ){ // this.resolveLink() inflates / expands the tree > search can be done again
			...
			if( p == null && !this.isTerminology){
				...
				if(url != null){
					Section term = TerminologyManager.instance().loadTerminology(url, this.getType());
					if(term != null){
	...
	public Vector<Property> getProperties(String name){
		if (name.contains("/")){ // name actually a path > calling method from section defined by path
			logger.info("name to look for properties actually a path");
			name = ensureValidPathEnding(name);
			Section actualSec = getActualSectionSpecifiedByPath(name.substring(0,name.lastIndexOf("/")), false);
	...
	public boolean hasTerminology(){
		if (this.terminology == null) return false;
		else {	return true;	} 
	}
	/**
	...
	/**
	 * Merges this section with another section. Generally all properties in the other section will be copied.
	 ...
	 * passed in otherSection .</li>
	 * 	<li>MERGE_OTHER_OVERRIDES_THIS: the global information passed with otherSection 
	 * overrides local information.</li>
	 * 	<li>MERGE_COMBINE: the local information will be combined with the passed section. 
	 * properties with the same name will be fused.</li>
	 * </ol>
	 * Merging works recursively. Only those subsections are merged that share the same name and type! Otherwise
	...
	public void merge(Section otherSection,int mergeOption){
		...
		if ((this.getRepository()!=null && otherSection.getRepository()!=null) && 
			...
			logger.error("Section.merge error: cannot merge sections based on different terminologies!");
			return;
		}
		...
		for(int i=0;i<otherSection.sectionCount();i++){
			Section temp = null;
			try {
				temp = (Section)otherSection.getSection(i).copy();
			} catch (Exception e) {
			...
			int index = this.contains(temp);
			if(index == -1){ //if 'otherSection' contains a new section add it.
				this.add(temp);
			}
		...
		//terminologyURL
		if(this.getRepository() ==null){this.setRepository(otherSection.getRepository());}
		else{//resolve conflict
			if(mergeOption==MERGE_OTHER_OVERRIDES_THIS &&
					otherSection.getRepository()!=null){
				this.setRepository(otherSection.getRepository());
			}
		}
		//Definition
		if(this.getDefinition() == null){this.setDefinition(otherSection.getDefinition());}
		else{//resolve conflict
		...
		//mappingURL
		if(this.getMapping() ==null){this.setMapping(otherSection.getMapping());}
		else{//resolve conflict
	...
	/**
	 * Checks whether or not the section already contains a property with the same 
	 * name, unit, type, parentValue and parent. Returns the index of the first matching property.
		...
		if(properties!=null){
			for(int i=0;i<properties.size();i++){
				if(properties.get(i).getName().equalsIgnoreCase(property.getName())){
					//					if((properties.get(i).getType()!=null && property.getType()!=null && properties.get(i).getType().equalsIgnoreCase(property.getType())) ||
					//							(properties.get(i).getType()==null && property.getType()==null)){
					//						if((properties.get(i).getUnit()!=null && property.getUnit()!=null && properties.get(i).getUnit().equalsIgnoreCase(property.getUnit())) ||
					//								(properties.get(i).getUnit()==null && property.getUnit()==null)){
					if((properties.get(i).getDependency()!=null && property.getDependency()!=null && properties.get(i).getDependency().equalsIgnoreCase(property.getDependency())) ||
							(properties.get(i).getDependency()==null && property.getDependency()==null)){
						if((properties.get(i).getDependencyValue()!=null && property.getDependencyValue()!=null && properties.get(i).getDependencyValue().equalsIgnoreCase(property.getDependencyValue())) ||
								(properties.get(i).getDependencyValue()==null && property.getDependencyValue()==null)){
							return i;
						}
						else{continue;}
					}	
	...
	/**
	 * Checks whether or not this section already contains a subsection with the same 
	 * name and type. Method does not traverse the subsections!
	 ...
	 * @return String the standard odML name of the property. Returns null if not found.
	 */
	 ...
	 * @param mapping {@link URL}: the url to which this section maps. Can (should)
	 * contain references to indicate the type of section to include.
	 */
	 ...
	 * @param mapping {@link String}: the url to which this property maps. Must not contain a refernce part.
	 */
	...
	public void validate(){
		...
		if(this.terminology== null && findRepositoryURL() == null){
			logger.warn("No terminology information available for section "+this.getPath()+"... skipping!");
			return;
		}
		if(terminology == null){
		...
		if(terminology==null){
			logger.error("No terminology information could be located or loaded! ... validation of section: "+this.getPath() +" skipped!");
			return;
		}
		if(this.definition != null && !this.definition.isEmpty()){
			if(!this.definition.equalsIgnoreCase(terminology.getDefinition())){
				logger.warn("The definition stored in section '"+this.getPath()+"' does not match the terminology definition." +
		...
		for(int i=0;i<propertyCount();i++){
			...
			if(termProp != null){
				...
				logger.info("Property: "+p.getName()+ " could not be validated. No counterpart in the terminology.");
			}
		}
	}
	/**
	 * This method scans the tree and optimizes linked sections. This means that local 
	 * sections are simplified that they only contain information that deviates from the 
	 * global description. 
	 */
	public void optimizeTree(){
		Section root = getRootSection();
		for (int i=0; i<root.sectionCount();i++){ // going through all sections
			root.getSection(i).optimize();
		}
		logger.info("optimization done");
	}
	/**
	 * Optimizes this section. Optimization is only done if this section is linked and all referring sections
	...
	public String toString(){
		...
		return	(this.type + "-section named '" + this.name + "'");
	}
	/**
	...
	public String toStringExtended(){
		String info = (this.type+"-section named '" + this.name + "', id ("+this.reference+") on level: " + level + "; complete path: "+
				this.getPath()+"\n\t- ");
	...
	/**
	 * Returns a detailed String representation of this section. I.e it's name, level, completePath 
	 * + definition, termURL, mappingURL
	 * + number and names of subsections and number and names of appended properties
	 */
	public String toStringAllDetails(){
		String info = (this.type+"-section named '" + this.name + "', id ("+this.reference+") on level: " + level + "; full path: "+
				this.getPath()+"\n\t- ");
	...
	/**
	 * Method for getting the parent = section where to add new Property / Section via a 
	 * path of sections. Different Sections / levels separated by "/"
	 * @param pathOfSections {@link String}: The path to specify the destination
	 * @param usedForInserting {@link boolean}: flag to specify whether method is used for
	 * adding or getting sec/prop via path
	 * @return Section {@link Section}: the section that is end of the path. created if 
	 * not existing yet (same for other not existing sections yet on path) 
	 */
	private Section getActualSectionSpecifiedByPath(String pathOfSections, boolean usedForAdding){
		...
		int pathDepth = pathPieces.length -1; //-1 as tree level begins at zero
		logger.debug("pathPieces with length: '"+pathPieces.length+
				"'; section for inserting prop/sec has level: "+pathDepth);
		if (pathDepth < 0) return null;
		...
		if ((this.level == pathDepth)&&(this.getPath() == pathOfSections)){
		...
		else { // check where to add the prop/sec; creating section(s) if not existing yet
			if (this.level > 0) {
				newParent = this.parent;
				while(newParent.level != 0){ // go to root first to follow path from there
					newParent = newParent.parent;
				}
			}
			//check whether root has name, if not go one level deeper to first path-piece
			...
			} // copying all besides first two, as [0] is empty string and [1] is already starting point

			newParent = newParent.followPath(pathPiecesFromRoot, false, usedForAdding);
		} // end determining parent of prop/sec to add
		logger.debug("Section specified by path has name: '"+newParent.type+"', and total path: '"+newParent.getPath()+"'");
		return newParent;
	}
	/**
	 * returning the section obtained by following the given path fragmented in an array. 
	 * possibility to ignore first entry (important when having "." in it. distinguishing 
	 * between adding and getting-case > for getting no sections must be created!
	 * @param pathPieces {@link String[]}: String array containing single path elements = section names
	 * @param givenIgnoreFirst {@link boolean}: flag to be able to ignore first entry (could be ".")
	 * @param usedForAdding {@link boolean}: flag for distinguishing between adding and getting-case;
	 * for getting-case no sections must be created (returned section null then)
	 * @return {@link Section} - returning the section specified by path.
	 * usedForAdding == true: creating sections on path when not existing yet
	 * usedForAdding == false: returning null when sections on path not existing 
	 */
	...
	/**
	 * This method returns the absolute path of this section beginning from the root. Paths are made up from section names separated
	 * by "/". Leading '/' indicates an absolute path.
	...
	public String getPath(){
		...
		//		if ((this.getParent() != null)){ // parent existing, recursive calling
		//			completePath = this.name;
		//			if ((((Section)this.getParent()).getName() != null) && (!((Section)this.getParent()).getName().isEmpty())){
		//				completePath = ((Section)this.getParent()).getPath().concat("/").concat(completePath);
		//			}
		//			else { // parent existing, but no name (means, is root, no other option possible) > just adding '/'
	...
	//*****
	// important methods to be able to create a TreeNode Enumeration (see below) containing Properties and Sections
	//*****
	private Vector<TreeNode> getTreeNodeSections(){
		...
		for (int i = 0; i<(this.subsections).size();i++){
			tnSections.add((TreeNode)this.subsections.elementAt(i));
		}
		return tnSections;
	}
	private Vector<TreeNode> getTreeNodeProperties(){
		...
		for (int i = 0; i<this.propertyCount();i++){
			tnProperties.add((TreeNode)this.getProperty(i));
		}
		return tnProperties;
	}
	//*************************************************************
	...
	public Enumeration<TreeNode> children() {
		...
		// creating an Enumeration of TreeNodes containing first the subsections, then the Properties
	...
	public TreeNode getChildAt(int arg0) {
		...
		if (arg0 < this.sectionCount()){
			return (TreeNode)getSection(arg0);
		}
		else {
			return (TreeNode)getProperty(arg0-this.sectionCount());
		}
	}
	@Override
	...
	public int getIndex(TreeNode arg0){
		if (arg0 instanceof Section){
			...
			for (int i = 0; i < this.sectionCount(); i++){
				if(this.getSection(i).equals(soo)) return i;
			}
			...
			for (int j = 0; j < this.propertyCount(); j++){
				// sectionCount must be added as Enumeration contains first the sections, then the properties
				if(this.getProperty(j).equals(poo)) return (j + this.sectionCount()); 
			}
	...
	/**
	 * Set a link. This indicates that this section is linked to another one and thus inherits its properties.
	 * Links can only be established between sections of the same type and are defined by the absolute paths in 
	 * the odml tree. Paths are the '/' separated names of sections, e.g. /dataset 1/myStimulus/. 
	...
	/**
	 * Set a link. A link indicates that this section is linked to another one and thus inherits its properties.
	 * Links can only be established between sections of the same type and are defined by the absolute paths in 
	 * the odml tree. Paths are the '/' separated names of sections, e.g. /dataset 1/myStimulus/. Using this function 
	 * a link can be set that is untested. 
	 * @param link String the link as an absolute path (beginning at the root section and starting with / 
	 * e.g. '/dataset 1/myStimulus/')
	 * @param ignore {@link Boolean} setting this true does not test the link for validity!
	 */
	public void setLink(String link, boolean ignore){
		...
		if(ignore){
			...
			if(!link.startsWith("/")){
				logger.warn("Section.setLink: A link must be given as an abolute path in the tree, i.e. start with '/'.");
				link = "/"+link;
			}
			Section temp = this.getSectionViaPath(link);
			if(temp == null){
				logger.error("Section.setLink: The link is invalid. Referenced section does not exist. Link: "+link);
				return;
			}
			if(!temp.getType().equalsIgnoreCase(this.type)){
				logger.error("Section.setLink: The link ("+link+")is invalid! Section types (this: "+this.type+"," +
	...
	/**
	 * Sets the include information of this section. The include string can represent an url or a relative path. 
	 * Setting the include information does not automatically load the resource. To do so call loadInclude() method. 
	 *
	 * @param include A {@link String} representing a valid url an absolute or relative path of the resource to included.
	 */
	...
	/**
	 * Sets the author of this document. Author elements are only valid in root sections.
	 * They are otherwise ignored. 
	 * @param author {@link String} the author of the document.
	 */
	public void setDocumentAuthor(String author) {
		if(!this.isRoot()){
			logger.warn("Author information is only allowed in Root sections. Otherwise it will be ignored.");
		}
		this.author = author;
	}
	/**
	...
	/**
	 * Set the Version of the document. Version elements are only valid in root sections.
	 * They are otherwise ignored. 
	 * @param version {@link String} the version of the document.
	 */
	public void setDocumentVersion(String version) {
		if(!this.isRoot()){
			logger.warn("Version information is only allowed in Root sections. Otherwise it will be ignored.");
		}
	...
	/**
	 * Loads an included file into this section and merges them while the locally defined information 
	 * (properties of this section) overwrites the imported one. 
	 */
	public void loadInclude(){
		...
		if(this.getInclude().contains("#")){
			...
			this.setInclude(this.getInclude().substring(0,this.getInclude().indexOf("#")));
		}
		...
		try{
			...
			try {
				...
				try {
					logger.error("Could not create a file from the include information: "+this.getRootSection().getFileUrl().toURI()+" combined with "+this.getInclude());
				} catch (Exception e2) {
					e2.printStackTrace();
				}
				return;
		...
		try {
			...
			logger.error("Could not read file from the include location: "+fileUrl);
			return;
		}
...
//			System.out.println(includeSectionName);
			includeSections = rootSection.findSections(includeSectionName);
//			System.out.println("found sections: "+includeSections.size());
...
//					System.out.println("delete one");
		...
		if(includeSections.size() >1 && (includeSectionName == null || includeSectionName.isEmpty())){
			logger.error("Could not include from indicated location: Include statement is ambiguous!");
		}
		else if(includeSections.size() > 1 ){
			for(int j=0;j<includeSections.size();j++){
//				System.out.println(includeSections.get(j).getName()+" compare with " +this.getName());
...
//					System.out.println("found");
			...
			logger.error("Could not find corresponding include section Include location does not contain section of type: "+this.getType());
			return;
		}
		if(includeSection!= null){
			...
			logger.error("Could not find corresponding include section. Inlcude location does not contain section of type: "+this.getType());
			return;
		}
		this.include = null;
	}
	/**
	 * Loads inlcudes from this section and cylces through all subsections. 
	 * If you want to load all includes in the tree call this function on the rootSection.
	 */
	...
	/**
	 * Sets the fileURL of the file this section originated from. This information is used to resolve includes if they are
	 * relative. Usually only present in the root section.
	 * @param fileUrl {@link URL} the URL of the original file.
	 */
	public void setFileUrl(URL fileUrl) {
		this.fileUrl = fileUrl;
	}
	/**
	 * Returns the stored file url, i.e. the url of the file this section originated from. Usually only defined for root sections.
	 * 
