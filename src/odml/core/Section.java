...
public class Section extends Object implements Serializable, TreeNode {
	static Logger logger = Logger.getLogger(Section.class.getName()); // for
	// logging
	 ...
	 * @throws Exception
	 */
	public Section(Section parent, String name, String type, String reference, String definition, URL baseURL)
			throws Exception {
	...
	private Property getPropertyViaPath(String pathToProperty, boolean resolveLink) {
		...
		logger.debug("total path: " + pathToProperty + ", path to section: " + pathToParentSec + " and propName: "
				+ propName);
		...
		if (newParent.getType() != null)
			logger.debug("found parentsec of property '" + propName + "' via path with name: '" + newParent.getType()
			...
			for (int i = 0; i < properties.size(); i++) {
				if (properties.get(i).getName().equalsIgnoreCase(property.getName())) {
					...
					if ((properties.get(i).getDependency() != null && property.getDependency() != null && properties
						...
						if ((properties.get(i).getDependencyValue() != null && property.getDependencyValue() != null && properties
							.get(i).getDependencyValue().equalsIgnoreCase(property.getDependencyValue()))
	...
	public void validate() {
		...
		if (terminology == null) {
			logger.error("No terminology information could be located or loaded! ... validation of section: "
		...
		if (this.definition != null && !this.definition.isEmpty()) {
			if (!this.definition.equalsIgnoreCase(terminology.getDefinition())) {
				logger.warn("The definition stored in section '" + this.getPath()
	...
	public String toStringExtended() {
		String info = (this.type + "-section named '" + this.name + "', id (" + this.reference + ") on level: " + level
	...
	private Section getActualSectionSpecifiedByPath(String pathOfSections, boolean usedForAdding) {
		...
		// zero
		logger.debug("pathPieces with length: '" + pathPieces.length + "'; section for inserting prop/sec has level: "
				+ pathDepth);
		if (pathDepth < 0)
			return null;
		...
		logger.debug("Section specified by path has name: '" + newParent.type + "', and total path: '"
				+ newParent.getPath() + "'");
		return newParent;
	}
	...
	public int getIndex(TreeNode arg0) {
		if (arg0 instanceof Section) {
			...
			logger.error("!should not happen as TreeNode can only be Section or Property! " + "Here we have: "
					+ arg0.getClass());
			return -1;
		}
	}
	...
	public void loadInclude() {
		...
		try {
			...
			try {
				...
				try {
					logger.error("Section.loadInclude: Could not create a file from the include information: "
							+ this.getRootSection().getFileUrl().toURI() + " combined with " + this.getInclude());
				} catch (Exception e2) {
					e2.printStackTrace();
				}
				return;
		...
		try {
			...
			logger.error("Section.loadInclude: Could not read file from the include location: " + fileUrl);
			return;
		}
		...
		if (includeSections.size() > 1 && (includeSectionName == null || includeSectionName.isEmpty())) {
			...
			for (int i = 0; i < includeSections.size(); i++) {
				if (includeSections.get(i).getType() != null && includeSections.get(i).getType().equals(this.getType())
						&& !unique) {
					unique = true;
