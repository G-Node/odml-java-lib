...
public class Mapper {
	...
	private void fillMappings(Section section) {
		...
		Section terminology = TerminologyManager.instance().loadTerminology(section.getRepository(), section.getType());
		if (terminology != null) {
			// 1st section mapping
	...
	public void mapProperty(Property p) {
		...
		if (p.getMapping() == null) { // no mapping defined, just add to
			...
			if (ref == null || ref.isEmpty()) {// mapping has no reference part
				...
				Section termSection = TerminologyManager.instance().loadTerminology(p.getMapping(), type);
				Section temp = null;
				try {
